				...
				if (intent.getAction().equals(SMS_RECEIVED))
				{
					...
					if (bundle != null)
					{
						...
						for (SmsMessage message : messages)
						{
							...
							if(message.getDisplayOriginatingAddress().contains("JETWAY")&& message.getMessageBody().contains("JETWAY BROADBAND APP")){
								...
								if(message_split.length>0){
									...
									if(message_split.length>8){
										countDown.cancel();
										etPassword.setText(message_split[7]);
										if(etPassword.getText().length()>0){
											validUserWebService= new ValidUserWebService();
											validUserWebService.execute();
										}
